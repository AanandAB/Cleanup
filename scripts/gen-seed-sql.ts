...
for (const p of projects) {
  sql +=
    `INSERT INTO projects (id, slug, title, location, type, description, images, sort_order, updated_at) ` +
    `VALUES (${q(randomUUID())}, ${q(p.slug)}, ${q(p.title)}, ${q(p.location)}, ${q(p.type)}, ${q(p.description)}, ${q(JSON.stringify(p.images))}, 0, unixepoch());\n`;
}
